...
void ImGui::Render()
{
    ...
    {
        ImU32 SpookCol = IM_COL32(239, 239, 211, 255);
        const ImVec2 pos = g.IO.MousePos - offset;
        const ImTextureID tex_id = g.IO.Fonts->TexID;
        const float sc = g.Style.MouseCursorScale;
        g.OverlayDrawList.PushTextureID(tex_id);
        g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(1,0)*sc, pos+ImVec2(1,0)*sc + size*sc, uv[2], uv[3], /*IM_COL32(0,0,0,48)*/SpookCol);        // Shadow
        g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(2,0)*sc, pos+ImVec2(2,0)*sc + size*sc, uv[2], uv[3], /*IM_COL32(0,0,0,48)*/SpookCol);        // Shadow
        g.OverlayDrawList.AddImage(tex_id, pos,                  pos + size*sc,                uv[2], uv[3], /*IM_COL32(0,0,0,255)*/ SpookCol);       // Black border
        g.OverlayDrawList.AddImage(tex_id, pos,                  pos + size*sc,                uv[0], uv[1], /*IM_COL32(255,255,255,255)*/SpookCol); // White fill
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderTextClipped(bb.Min + style.FramePadding/1.5f , bb.Max - style.FramePadding/1.5f, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    g.FontSize = font;
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBg : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ...
    {
        if(*v >= (v_min+v_max)/2)
        window->DrawList->AddRectFilled(ImVec2((frame_bb.Min.x+frame_bb.Max.x)/2,frame_bb.Min.y),ImVec2(grab_bb.Max.x,frame_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        else
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x,frame_bb.Min.y),ImVec2((frame_bb.Min.x+frame_bb.Max.x)/2,frame_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    }
    return value_changed;
}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    const ImRect meme_bb(frame_bb.Min, frame_bb.Max - ImVec2(0, style.FramePadding.y*3.2f));
    // NB- we don't call ItemSize() yet because we may turn into a text edit box below
    if (!ItemAdd(total_bb, id,&meme_bb))
    {
        ItemSize(total_bb, style.FramePadding.y);
        g.FontSize = font;
        return false;
    }
    const bool hovered = ItemHoverable(meme_bb, id);
    ...
    ItemSize(total_bb, style.FramePadding.y);
    const bool value_changed = SliderBehavior(meme_bb, id, v, v_min, v_max, power, decimal_precision);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), display_format, *v);
    RenderTextClipped(ImVec2(frame_bb.Min.x,meme_bb.Max.y),total_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y ,label_size.y)); // We want a square shape to we use Y twice
    ItemSize(check_bb, style.FramePadding.y);

    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y * 0.5f),
                         window->DC.CursorPos + ImVec2(0, style.FramePadding.y * 0.5f) + label_size);
    if (label_size.x > 0) {
...
static bool BeginChildExTitle(const char* name, ImGuiID id, const char* titlebar, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* parent_window = ImGui::GetCurrentWindow();
    ImGuiWindowFlags flags = ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoSavedSettings|ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_NoCollapse;
    flags |= (parent_window->Flags & ImGuiWindowFlags_NoMove);  // Inherit the NoMove flag
    ...
    ImGui::SetNextWindowSize(size);
    bool ret = ImGui::Begin(titlebar, NULL, flags);
    ImGuiWindow* child_window = ImGui::GetCurrentWindow();
    child_window->AutoFitChildAxises = auto_fit_axises;
    if (!(parent_window->Flags &  ImGuiWindowFlags_Modal))
        child_window->Flags &= ~ ImGuiWindowFlags_Modal;

    return ret;
}

bool ImGui::BeginChildwithTitlebar(const char* str_id, const char* title ,const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return BeginChildExTitle(str_id, window->GetID(str_id), title,size_arg, border, extra_flags);
}

bool ImGui::BeginChildwithTitlebar(ImGuiID id, const char* title ,const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    return BeginChildExTitle(NULL, id,title, size_arg, border, extra_flags);
}
...
int ImGui::PiePopupSelectMenu( const ImVec2& center, const char* popup_id, const char** items, int items_count , ImColor col1 , ImColor col2 , ImColor col3 ) {
    ...
    if ( ImGui::BeginPopup( popup_id ) ) {
        ...
        for ( int item_n = 0; item_n < items_count; item_n++ ) {
            ...
            ImVec2 text_size = ImGui::GetFont()->CalcTextSizeA( ImGui::GetFont()->FontSize, FLT_MAX, 0.0f,
                                                                item_label );
...
bool ImGui::TextButton(const char* label, float sizemultiply , bool* v, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect bb(pos, pos + size*sizemultiply/*mEmE*/);
    ItemSize(bb, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::KeyButton(const char* label, const ImVec2& size_arg)
    {
    ...
    return false;
        ...
        RenderTextClipped(ImVec2(bb.Min.x,bb.Min.y+(bb.Max.y-bb.Min.y)*0.25f), bb.Max , label, NULL, &label_size, style.ButtonTextAlign, &bb);
